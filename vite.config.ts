...
function miaodaOutputPlugin(): Plugin {
  return {
    ...
    closeBundle() {
      ...
      // assets → CDN 桶
      // 注：部分 Windows 环境下 Node 的 fs.cpSync 递归复制会导致进程异常退出
      // （0xC0000409），改为逐文件手动复制，避免收尾崩溃影响产物完整性。
      const assets = path.join(client, 'assets');
      if (fs.existsSync(assets)) {
        copyDirRecursive(assets, path.join(outputResource, 'assets'));
      }
...
// 逐文件递归复制目录，规避 Windows 上 fs.cpSync 的进程崩溃问题
...
export default defineConfig(({ command }) => ({
  ...
  server: {
    ...
    port: 31873,
    proxy: {
      // 本地开发时代理到 DeepSeek，绕开浏览器 CORS。前端请求 /llm-proxy/...
      '/llm-proxy': {
